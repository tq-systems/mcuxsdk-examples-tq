...
/*  SRC GPR1 as M33 active status flag, A55 will read the value before enter
 * suspend mode */
 ...
 * A55 will set GPR2 bit1 when it will enter suspend, and clear it before
 * wakeup. bit 2 indicates that M33 has made the appropriate settings allowing
 * A55 to configure PLLs in LPM auto mode.
 */
...
#define dwc_ddrphy_apb_wr(addr, data)                                          \
  W32(IP2APB_DDRPHY_IPS_BASE_ADDR(0) + BOARD_DDRPHY_Addr_Remap(addr), data)
#define dwc_ddrphy_apb_rd(addr)                                                \
  R32(IP2APB_DDRPHY_IPS_BASE_ADDR(0) + BOARD_DDRPHY_Addr_Remap(addr))
 ...
 * Acore run in 64 bit system, Mcore run in 32 bit system. So, add reserved area
 * in dram_timing_info to protect the data from being matched correctly data
 * match.
 */
