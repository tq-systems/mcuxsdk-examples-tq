 ...
 * Author: Isaac Lucas de Lima Yuki, Bernhard Herz
 */
//******************************************************************************
...
static lpi2c_master_transfer_t i2c_config = {
  .flags          = kLPI2C_TransferDefaultFlag,
  .slaveAddress   = PMIC_DEVICE_ADDRESS,
  .dataSize       = PMIC_REG_DATA_SIZE,
  .subaddressSize = PMIC_REG_ADDRESS_SIZE};
 ...
 * is called.
 */
status_t PMIC_getCoreVoltage(float *voltage)
{
  status_t status;
 ...
 * is called.
 */
status_t PMIC_getDeviceID(uint8_t *deviceId)
{
  status_t status;
