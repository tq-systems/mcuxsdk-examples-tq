...
void BOARD_InitDebugConsole(void)
{
  ...
  PRINTF("Branch: %s \r\n", BRANCH);
#endif /* VERSION_INFORMATION */
  PRINTF("This is a demo application for the " BOARD_NAME " device.\r\n");
}
...
status_t BOARD_LPI2C_Send(LPI2C_Type *base, uint8_t deviceAddress,
                          ...
                          uint8_t *txBuff, uint8_t txBuffSize, uint32_t flags)
{
...
void BOARD_ConfigMPU(void)
{
  ...
  /*
   * default cache policy(default memory access behavior) after enable mpu on
   * cortex-m33(according to RM of cortex-m33): 0x00000000-0x1FFFFFFF Normal
   * memory, Non-shareable, Write-Through, not Write Allocate
   * 0x20000000-0x3FFFFFFF Normal memory, Non-shareable, Write-Back, Write
   * Allocate 0x40000000-0x5FFFFFFF Device, Shareable 0x60000000-0x7FFFFFFF
   * Normal memory, Non-shareable, Write-Back, Write Allocate
   * 0x80000000-0x9FFFFFFF Normal memory, Non-shareable, Write-Through, not
   * Write Allocate 0xA0000000-0xDFFFFFFF Device, Shareable
   * 0xE0000000-0xE003FFFF Device, Shareable 0xE0040000-0xE0043FFF Device,
   * Shareable 0xE0044000-0xE00EFFFF Device, Shareable 0xF0000000-0xFFFFFFFF
   * Device, Shareable
   */
  /* Disable MPU */
  ARM_MPU_Disable();
  ...
  ARM_MPU_SetMemAttr(0U,
                     ARM_MPU_ATTR(ARM_MPU_ATTR_DEVICE, ARM_MPU_ATTR_DEVICE));

  /* Attr1: Normal memory, Outer non-cacheable, Inner non-cacheable */
  ARM_MPU_SetMemAttr(
    1U, ARM_MPU_ATTR(ARM_MPU_ATTR_NON_CACHEABLE, ARM_MPU_ATTR_NON_CACHEABLE));

  /* Attr2: Normal memory, Inner write-through transient, read allocate. Inner
   ...
   * Inner write-back transient, read/write allocate */
  attr = ARM_MPU_ATTR_MEMORY_(0U, 1U, 1U, 1U);
  ARM_MPU_SetMemAttr(3U, ARM_MPU_ATTR(attr, attr));

  /*
   * Change macro definitions as follows when choose cache policy as
   * non-cacheable: #define DDR_NONCACHEABLE (1U) #define DDR_WRITE_THROUGH (0U)
   * #define DDR_WRITE_BACK (0U)
   *
   *
   * Change macro definitions as follows when choose cache policy as
   * Write-Through: #define DDR_NONCACHEABLE (0U) #define DDR_WRITE_THROUGH (1U)
   * #define DDR_WRITE_BACK (0U)
   *
   *
...
#if DDR_NONCACHEABLE
  /* NOTE: DDR is used as shared memory for A/M core communication, set it to
   * non-cacheable. */
  /* Region 0: [0x80000000, 0xDFFFFFFF](DRAM), outer shareable, read/write, any
   * privileged, executable. Attr 1 (non-cacheable). */
  ARM_MPU_SetRegion(0U, ARM_MPU_RBAR(0x80000000, ARM_MPU_SH_OUTER, 0U, 1U, 0U),
                    ARM_MPU_RLAR(0xDFFFFFFF, 1U));
#elif DDR_WRITE_THROUGH
  /* Region 0: [0x80000000, 0xDFFFFFFF](DRAM), outer shareable, read/write, any
   * privileged, executable. Attr 2 (Normal memory, Inner write-through
   * transient, read allocate. Inner write-through transient, read allocate). */
  ARM_MPU_SetRegion(0U, ARM_MPU_RBAR(0x80000000, ARM_MPU_SH_OUTER, 0U, 1U, 0U),
                    ARM_MPU_RLAR(0xDFFFFFFF, 2U));
#elif DDR_WRITE_BACK
  /* Region 0: [0x80000000, 0xDFFFFFFF](DRAM), outer shareable, read/write, any
   * privileged, executable. Attr 3 (Normal memory, Outer write-back transient,
   * read/write allocate. Inner write-back transient, read/write allocate). */
  ARM_MPU_SetRegion(0U, ARM_MPU_RBAR(0x80000000, ARM_MPU_SH_OUTER, 0U, 1U, 0U),
                    ARM_MPU_RLAR(0xDFFFFFFF, 3U));
#endif /* DDR_NONCACHEABLE / DDR_WRITE_THROUGH / DDR_WRITE_BACK */
...
static unsigned long BOARD_DDRPHY_Addr_Remap(uint32_t paddr_apb_from_ctlr)
{
  ...
  /* seacrching backward, so the last tested addr (0) is also the default value
   * in case not found */
  paddr_apb_unqual_dec_19_13 = DDRPHY_NB_ADDR_REMAP - 1;
  while ((paddr_apb_unqual_dec_19_13 > 0)
         && (BOARD_DDRPHY_Addr_Remap_table[paddr_apb_unqual_dec_19_13]
             != (uint16_t) paddr_apb_unqual_dec_22_13))
  {
    paddr_apb_unqual_dec_19_13--;
  }

  paddr_apb_unqual =
    ((paddr_apb_unqual_dec_19_13 << 13) | (paddr_apb_unqual_dec_12_1 << 1));
...
void BOARD_DDRPHY_ClodRest(void)
{
  ...
  SRC_DPHY_SLICE->SLICE_SW_CTRL &=
    ~SRC_MIX_SLICE_SLICE_SW_CTRL_RST_CTRL_SOFT_MASK;
  /* src_gen_dphy_sw_rst_de_assert */
  SRC_DPHY_SLICE->SINGLE_RESET_SW_CTRL &=
    ~SRC_MIX_SLICE_SINGLE_RESET_SW_CTRL_RST_CTRL_SOFT_1_MASK;
  /* src_gen_dphy_PwrOKIn_sw_rst_de_assert() */
  SRC_DPHY_SLICE->SINGLE_RESET_SW_CTRL |=
    SRC_MIX_SLICE_SINGLE_RESET_SW_CTRL_RST_CTRL_SOFT_0_MASK;
...
void BOARD_DDRC_Restore(struct dram_timing_info *timing)
{
  ...
  waitus = BOARD_Check_DDRC_Idle(DDRC_DDRDSR_2_Check_Idel_time,
                                 DDRC_DDRDSR_2_IDLE_MASK);

  if (waitus == -1)
  {
    PRINTF("Check DDRC Idle status fail\r\n");
  }
}
...
void BOARD_DRAMEnterRetention(void)
{
  ...
  /* Set BLK_CTRL_DDRMIX AUTO_CG_CTRL[HWFFC_ACG_FORCE_B], enable ipg_stop_reg
   * while auto cg */
  BLK_CTRL_DDRMIX->AUTO_CG_CTRL |=
    DDRMIX_BLK_CTRL_AUTO_CG_CTRL_HWFFC_ACG_FORCE_B_MASK;
  ...
  BLK_CTRL_DDRMIX->DDRC_STOP_CTRL |=
    DDRMIX_BLK_CTRL_DDRC_STOP_CTRL_DDRC_STOP_MASK;
  ...
  /* Set DDR_SDRAM_MD_CNTL[CKE_CNTL] to 0x00100000(this forces CKE to remain
   * low) */
  ...
  SDK_DelayAtLeastUs(10, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
  SRC_MIX_SLICE->SLICE_SW_CTRL |= SRC_MIX_SLICE_SLICE_SW_CTRL_PDN_SOFT_MASK;
  SDK_DelayAtLeastUs(10, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
}
